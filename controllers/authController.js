...
exports.login = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({ email }).select('+password'); //the + herer because we want to select a field it is not selected
...
// we want  to passa an array of allowed users who can access the resource
// and we can not pass arguments to middleware function so we will create a
// wrapper function that return the middleware function we want to create
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  const resetURL = `${req.protocol}://${req.get(
    'host'
  )}/api/v1/users/reswtPassword/${resetToken}`;
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({
    ...
    passwordResetExpires: { $gt: Date.now() }, //check if the token expires or it still valid
  });
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  //Check if the current password that the user enter is correct
